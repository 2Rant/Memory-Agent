...
# MODIFIED_MEMREADER_PROMPT = f"""You are a Personal Information Organizer, specialized in accurately storing facts, user memories, and preferences. Your primary role is to extract relevant pieces of information from conversations and organize them into distinct, manageable facts. This allows for easy retrieval and personalization in future interactions. Below are the types of information you need to focus on and the detailed instructions on how to handle the input data.
...
# - Supplementary Details: The `details` list must act as metadata to supplement the fact (e.g., Location, Price, Platform, Reason), not just splitting the fact's words.
# - Context Propagation: Ensure every extracted fact is self-contained. If a shared context (e.g., location, platform, activity) is established anywhere in the input chunk, explicitly include it in the `details` of all relevant facts, even if not repeated in every sentence.
...
# def get_facts_from_chunk(client, chunk, prompt):
#     """
...
#         print(f"Error extracting facts from chunk: {e}")
#         return []
...
#                 # Extract facts using modified prompt
#                 facts = get_facts_from_chunk(client, chunk, prompt)
...
#                 if chunk_facts:
#                     instance_data["facts_of_chunks"].append(chunk_facts)
...
#     # File paths
#     input_file = "/mnt/afs/codes/ljl/Memory-Agent/data/memalpha-train.jsonl"
#     output_file = "/mnt/afs/codes/ljl/Memory-Agent/data/memalpha-facts-gpt4_1.json"
...
def process_mem_alpha_dataset(input_file, output_file, client, prompt):
    ...
    for line_num, line in tqdm(enumerate(lines, 1), total=total_lines, desc="Processing lines"):
        try:
            ...
            # Fix fields with escape characters (chunks, metadata, etc.)
            fields_to_fix = ['chunks', 'metadata', 'questions_and_answers']
                    ...
                    if isinstance(value, str):
                        ...
                        if (value.startswith('[') and value.endswith(']')) or (value.startswith('{') and value.endswith('}')):
                            try:
                                ...
                                # If parsing fails, keep the cleaned string
                                data[field] = value
            
            # Ensure chunks is always a list
            chunks = data.get('chunks', [])
            if not isinstance(chunks, list):
                data['chunks'] = [chunks] if chunks else []
...
# def fix_existing_output(output_file):
#     """
#     Fix an existing output file with too many escape characters.
#     """
#     import json
#     print(f"Fixing existing output file: {output_file}")
...
#         print(f"Successfully fixed escape characters in {output_file}")
#     except Exception as e:
#         print(f"Error fixing output file: {e}")

if __name__ == "__main__":
    # File paths
    input_file = "/mnt/afs/codes/ljl/Memory-Agent/data/memalpha-test-filtered.jsonl"
    output_file = "/mnt/afs/codes/ljl/Memory-Agent/data/memalpha-facts-QA-test-4.1-0104.json"
    
    # # Check if output file exists and needs fixing
    # import os
    # if os.path.exists(output_file):
    #     print(f"Output file {output_file} already exists.")
    #     choice = input("Do you want to fix its escape characters? (y/n): ")
    #     if choice.lower() == 'y':
    #         fix_existing_output(output_file)
    #         exit(0)
